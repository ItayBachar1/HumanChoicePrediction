...
def history_and_llm_stochastic(history_window, quality_threshold, generated_scores, use_statistics):
    def func(information):

        # If 3 rounds have not yet passed, determine whether to go,
        # based on the average score between the score of the bot and the score of the llm model
        avg_score = (information["bot_message"] + generated_scores[information["review_id"]]) / 2
        if len(information["previous_rounds"]) < max(1, history_window):
            return int(avg_score >= 10 * np.random.rand())
        ...
        # otherwise the bot returns a score greater than or equal to the average.
                      ...
                      (r[HISTORY_FEATURES]['last_didGo_False'] and r[BOT_ACTION] >= r[REVIEWS].mean())) for r in
                     information["previous_rounds"][1:]]) == 1:
            if information["previous_rounds"][-1][HISTORY_FEATURES]['didGo'] and information["bot_message"] >= quality_threshold:
                return 1
            elif not information["previous_rounds"][-1][HISTORY_FEATURES]['didGo'] and information["bot_message"] < quality_threshold:
                return 0
        ...
        # dm identifies an action pattern of the bot based on the dm wining history
                ...
                r[HISTORY_FEATURES]['last_didWin_False'] and r[BOT_ACTION] >= r[REVIEWS].mean())) for r in
                     information["previous_rounds"][1:]]) == 1:
            if information["previous_rounds"][-1][HISTORY_FEATURES]['didWin'] and information["bot_message"] >= quality_threshold:
                return 1
            elif not information["previous_rounds"][-1][HISTORY_FEATURES]['didWin'] and information["bot_message"] < quality_threshold:
                return 0
